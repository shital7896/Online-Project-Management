...
export const postData = (formData,navigate) => async (dispatch) => {
    try {
        let response = await axios.post('http://localhost:8080/project/add', {
            ...formData
        });
...
export const getAllData = (page,limit,text,sort) => async (dispatch) => {
    try {
        ...
        if(text!==""){
            response = await axios.get(`http://localhost:8080/project/${text}?sort=${sort}&page=${page}&limit=${limit}`);
        }
        else{
            response = await axios.get(`http://localhost:8080/project?sort=${sort}&page=${page}&limit=${limit}`);
        }
...
export const updateData = (id,obj,page,limit) => async (dispatch) => {
    try{
       let response = await axios.patch(`http://localhost:8080/project/edit/${id}`,obj);
       let data = await response.data;
       console.log(data);
...
export const getAllCounts = () => async (dispatch) => {
    try{
     let response = await axios.get('http://localhost:8080/project/counts');
...
export const getChartsData = () => async (dispatch) => {
    try{
        let response = await axios.get('http://localhost:8080/project/charts');
        let data = await response.data;

                        ...
                        </Box>
                        {
                            screenSize.width > 640 ?
                                <Box display={'flex'} gap={'10px'} alignItems={'center'}>
                                    <Box>Sort By :</Box>
                                    <Select variant='simple' fontWeight={'bold'} width={'110px'} value={sort} onChange={(e) => handleSort(e)}>
                ...
                <DrawerContent>
                    ...
                    <DrawerBody lineHeight={"2.5"}>
                        <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
                            <Box id="priority" onClick={(e) => handleSelect(e)}>Priority</Box>
                            <Box id="category" onClick={(e) => handleSelect(e)}>Category</Box>
                            <Box id="location" onClick={(e) => handleSelect(e)}>Location</Box>
                            <Box id="division" onClick={(e) => handleSelect(e)}>Division</Box>
                            <Box id="department" onClick={(e) => handleSelect(e)}>Department</Box>
                            <Box id="reason" onClick={(e) => handleSelect(e)}>Reason</Box>
                            <Box id="status" onClick={(e) => handleSelect(e)}>Status</Box>
                        </SimpleGrid>
                    </DrawerBody>
                </DrawerContent>
            </Drawer>
                    ...
                    {
                        screenSize.width > 640 ?
                        <TableContainer mt={'5px'} width={'100%'} height={'auto'}>
                        <Table variant='simple'>
                            <Thead>
                                <Tr backgroundColor={'#EBF5FF'}>
                                    <Th style={{ textTransform: 'capitalize', fontSize: '14px', fontWeight: 'bold', color: '#3F3F3F' }}>Project Name</Th>
                                    <Th style={{ textTransform: 'capitalize', fontSize: '14px', fontWeight: 'bold', color: '#3F3F3F' }}>Reason</Th>
                                    <Th style={{ textTransform: 'capitalize', fontSize: '14px', fontWeight: 'bold', color: '#3F3F3F'}}>Type</Th>
                                    <Th style={{ textTransform: 'capitalize', fontSize: '14px', fontWeight: 'bold', color: '#3F3F3F' }}>Division</Th>
                                    <Th style={{ textTransform: 'capitalize', fontSize: '14px', fontWeight: 'bold', color: '#3F3F3F' }}>Category</Th>
                                    <Th style={{ textTransform: 'capitalize', fontSize: '14px', fontWeight: 'bold', color: '#3F3F3F' }}>Priority</Th>
                                    <Th style={{ textTransform: 'capitalize', fontSize: '14px', fontWeight: 'bold', color: '#3F3F3F' }}>Dept.</Th>
                                    <Th style={{ textTransform: 'capitalize', fontSize: '14px', fontWeight: 'bold', color: '#3F3F3F' }}>Location</Th>
                                    <Th style={{ textTransform: 'capitalize', fontSize: '14px', fontWeight: 'bold', color: '#3F3F3F' }}>Status</Th>
                                    <Th></Th>
                                </Tr>
                            </Thead>
                            <Tbody>
                                {data?.map((el) => (
                                    <Tr key={el._id} fontSize={'14px'}>
                                        <Td>
                                            <Text fontWeight={'bold'} color={'#414950'} fontSize={'16px'} wordBreak={'break-word'}>
                                                {el.theme}
                                            </Text>
                            ...
                            <Box marginTop={"20px"}>
                            {
                                data?.map((el) => (
                                    <Card
                                        ...
                                        key={el._id}
                                    >
                                        <Flex justifyContent={"space-between"} textAlign={"start"} flexDirection={{ base: 'column', sm: 'row' }}>
                                            <Box mb={{ base: "10px", sm: "0" }}>
                                                <Text fontWeight={"bold"} fontSize={{ base: "16px", sm: "18px" }}>{el.theme}</Text>
                                                <Text fontSize={{ base: "14px", sm: "16px" }} color={"gray"}>{el.start_date} to {el.end_date}</Text>
                                            </Box>
                                            <Text fontWeight={"bold"} fontSize={{ base: "16px", sm: "18px" }} color={"blue.700"}>{el.status}</Text>
                                        </Flex>
                                        <Flex fontSize={{ base: "14px", sm: "16px" }} mt={"10px"}>
                                            <Text color={"gray"}>Reason</Text>
                                            <Text>: {el.reason}</Text>
                                        </Flex>
                                        <Flex fontSize={{ base: "14px", sm: "16px" }} mt={"2px"} gap={"5px"}>
                                            <Flex>
                                        ...
                                        <Flex fontSize={{ base: "14px", sm: "16px" }} gap={"5px"} mt={"2px"}>
                                            <Flex>
                                        ...
                                        <Flex fontSize={{ base: "14px", sm: "16px" }} mt={"2px"}>
                                        ...
                                        <Flex mt={"10px"} justifyContent={"space-between"} flexDirection={{ base: 'column', sm: 'row' }}>
                                            <Flex gap={"5px"} justifyContent={{ base: 'center', sm: 'space-between' }} width={{ base: '100%', sm: 'auto' }}>
                                                <button className='btn-start' onClick={() => handleStart(el._id)}>Start</button>
                                                <button className='btn-close' onClick={() => handleClose(el._id)}>Close</button>
                                                <button className='btn-cancel' onClick={() => handleCancel(el._id)}>Cancel</button>
                                            </Flex>
